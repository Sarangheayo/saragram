...
async function socialKakao(code) {
  ...
  const resultToken  = await axios.post(
    process.env.SOCIAL_KAKAO_API_URL_TOKEN, tokenRequest.searchParams, { headers: tokenRequest.headers });
  ...
  const refreshToken = db.sequelize.transaction(async t => {
    ...
    return refreshToken; // from sara server refresh token
}
